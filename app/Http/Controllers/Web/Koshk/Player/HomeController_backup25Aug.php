...
class HomeController extends KoshkController
{
    ...
    public function index()
    {
    	...
    	$urldata = $this->getInput("data");
    	
    	if(strlen($urldata) > 3)
    	{

			try {
			    $de_data = decrypt($urldata);
		...
		if(!$this->missing_input)
		{
			...
			return view('koshk.player.home', compact('today', 'story', 'fname', 'lname', 'fid', 'book', 'fav', 'fav_count','data', 'count2', 'showifone'));  
		}
		else
		{
			//$request->session()->get('currentUser');
			//$user = Auth::user();
			//echo $user['first_name']. '   ' . $user['last_name']. '    '. $fid . '   '. $user['fid'];
			redirect()->route('readcomics');
		}
    }
