...
class FBController extends Controller
{
    ...
    public function __construct(UserRepository $users, RoleRepository $roles, Guard $auth)
    {
    ...
    private function getUserFromFacebook()
    {
		...
		if (! isset($accessToken)) {
		  ...
		  $loginUrl = $helper2->getLoginUrl('https://apps.facebook.com/koshkcomics/', $permissions);
    ...
    {
        //dd($request);
        $request->session()->put('fromFacebook', 1);
    	...
    	*/
        ...
        return $this->loginAndRedirect($user, $fid);
    }
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
    private function loginAndRedirect($user, $fid)
    {
        //dd($user);
        ...
        /*
        $data = ['book' => 11, 'id' => 7, 'fname' => $user['first_name'], 'fid' => $fid, 'lname' => $user['last_name']];
        ...
        return redirect()->action(
            'Koshk\Player\HomeController@index', ['data' => $en_data ]
        );*/
        ...
        if(Auth::check())
            {
            return redirect()->action(
                'Koshk\Player\HomeController@index', ['book' => 11, 'id' => 7, 'fname' => $user['first_name'], 'fid' => $fid, 'lname' => $user['last_name'], 'data' => 0 ]
            );
        } else 
        {
